...
class TestVerifierConstruction:
    ...
    def test_load_key(self):
        verifier = Verifier(open(PUBLIC_KEY, "rb").read())
        verifier.load_key(open(OTHER_PUBLIC_KEY, "rb").read())

    def test_load_key_file(self):
        verifier = Verifier(open(PUBLIC_KEY, "rb").read())
        verifier.load_key_file(OTHER_PUBLIC_KEY)
...
class TestEmptyVerifier:
    ...
    def test_empty_verifier_load_key(self):
        verifier = Verifier()
        verifier.load_key(open(PUBLIC_KEY, "rb").read())
        pkg = Package.open(RPM_SIGNED)
        pkg.verify_signature(verifier)
