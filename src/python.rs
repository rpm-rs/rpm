...
///
/// `members` is a slice of `(name, value)` pairs.
...
impl PyDependency {
    ...
    // TODO: Expose individual DependencyFlags bits as Python-friendly boolean properties
    //       (is_less, is_greater, is_equal, is_rpmlib, is_config, is_pre, is_post, etc.).
    #[getter]
    fn flags(&self, py: Python<'_>) -> PyResult<Py<PyAny>> {
...
impl PyScriptlet {
    ...
    // TODO: Expose individual ScriptletFlags bits as Python-friendly boolean properties.
    #[getter]
...
impl PyPackageMetadata {
    ...
    // --- Raw tag access (main header) ---

    /// Check whether the main header contains an entry for the given tag number.
    fn entry_is_present(&self, tag: u32) -> bool {
        self.0.header.entry_is_present(tag)
    }

    /// Look up a raw tag by number in the main header and return its data as a native Python type.
    ///
    ...
    fn entry(&self, tag: u32, py: Python<'_>) -> PyResult<Py<PyAny>> {
        let data = self.0.header.entry(tag).map_err(to_pyerr)?;
        index_data_to_py(py, data)
    }

    /// Return all entries in the main header as a dict mapping tag numbers to values.
    fn get_all_entries(&self, py: Python<'_>) -> PyResult<Py<PyAny>> {
        let entries = self.0.header.get_all_entries().map_err(to_pyerr)?;
    ...
    // --- Raw tag access (signature header) ---

    /// Check whether the signature header contains an entry for the given tag number.
    fn signature_entry_is_present(&self, tag: u32) -> bool {
        self.0.signature.entry_is_present(tag)
    }

    /// Look up a raw tag by number in the signature header and return its data as a native Python type.
    ///
    /// Returns `None` for Null, `bytes` for binary data, `int` or `list[int]`
    /// for integer types, `str` for single strings, and `list[str]` for string
    /// arrays. Raises `RuntimeError` if the tag is not present.
    fn signature_entry(&self, tag: u32, py: Python<'_>) -> PyResult<Py<PyAny>> {
        let data = self.0.signature.entry(tag).map_err(to_pyerr)?;
        index_data_to_py(py, data)
    }

    /// Return all entries in the signature header as a dict mapping tag numbers to values.
    fn get_all_signature_entries(&self, py: Python<'_>) -> PyResult<Py<PyAny>> {
        let entries = self.0.signature.get_all_entries().map_err(to_pyerr)?;
        let dict = PyDict::new(py);
        for (tag, data) in entries {
            dict.set_item(tag, index_data_to_py(py, data)?)?;
        }
...
impl PyPackage {
    ...
    ///
    /// If ``path`` is an existing directory, the package is written with an
    /// auto-generated filename based on its NEVRA (e.g.
    /// ``foo-1.0.0-1.x86_64.rpm``).  Otherwise ``path`` is treated as a file
...
impl PyDigestReport {
    ...
    #[getter]
    fn sha1_header(&self) -> PyDigestStatus {
        PyDigestStatus(self.0.sha1_header.clone())
    }

    /// SHA-256 of the header.
    #[getter]
    fn sha256_header(&self) -> PyDigestStatus {
        PyDigestStatus(self.0.sha256_header.clone())
    }

    /// SHA3-256 of the header (v6 packages).
    #[getter]
    fn sha3_256_header(&self) -> PyDigestStatus {
        PyDigestStatus(self.0.sha3_256_header.clone())
    }
...
pub fn rpm_rs(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add(
        ...
        make_int_enum(
            ...
            &[
                ...
                ("HEADERSIGNATURES", crate::IndexTag::RPMTAG_HEADERSIGNATURES as u32),
                ("HEADERIMMUTABLE", crate::IndexTag::RPMTAG_HEADERIMMUTABLE as u32),
                ("HEADERREGIONS", crate::IndexTag::RPMTAG_HEADERREGIONS as u32),
                ("HEADERI18NTABLE", crate::IndexTag::RPMTAG_HEADERI18NTABLE as u32),
                ...
                ("LONGARCHIVESIZE", crate::IndexTag::RPMTAG_LONGARCHIVESIZE as u32),
                ("SHA256HEADER", crate::IndexTag::RPMTAG_SHA256HEADER as u32),
                ("VERITYSIGNATURES", crate::IndexTag::RPMTAG_VERITYSIGNATURES as u32),
                ("VERITYSIGNATUREALGO", crate::IndexTag::RPMTAG_VERITYSIGNATUREALGO as u32),
                ("OPENPGP", crate::IndexTag::RPMTAG_OPENPGP as u32),
                ("SHA3_256_HEADER", crate::IndexTag::RPMTAG_SHA3_256_HEADER as u32),
                ...
                ("FILEGROUPNAME", crate::IndexTag::RPMTAG_FILEGROUPNAME as u32),
                ...
                ("FILEVERIFYFLAGS", crate::IndexTag::RPMTAG_FILEVERIFYFLAGS as u32),
                ...
                ("REQUIREVERSION", crate::IndexTag::RPMTAG_REQUIREVERSION as u32),
                ("NOSOURCE", crate::IndexTag::RPMTAG_NOSOURCE as u32),
                ("NOPATCH", crate::IndexTag::RPMTAG_NOPATCH as u32),
                ("CONFLICTFLAGS", crate::IndexTag::RPMTAG_CONFLICTFLAGS as u32),
                ("CONFLICTNAME", crate::IndexTag::RPMTAG_CONFLICTNAME as u32),
                ("CONFLICTVERSION", crate::IndexTag::RPMTAG_CONFLICTVERSION as u32),
                ("DEFAULTPREFIX", crate::IndexTag::RPMTAG_DEFAULTPREFIX as u32),
                ("BUILDROOT", crate::IndexTag::RPMTAG_BUILDROOT as u32),
                ("INSTALLPREFIX", crate::IndexTag::RPMTAG_INSTALLPREFIX as u32),
                ("EXCLUDEARCH", crate::IndexTag::RPMTAG_EXCLUDEARCH as u32),
                ("EXCLUDEOS", crate::IndexTag::RPMTAG_EXCLUDEOS as u32),
                ("EXCLUSIVEARCH", crate::IndexTag::RPMTAG_EXCLUSIVEARCH as u32),
                ...
                ("TRIGGERSCRIPTS", crate::IndexTag::RPMTAG_TRIGGERSCRIPTS as u32),
                ("TRIGGERNAME", crate::IndexTag::RPMTAG_TRIGGERNAME as u32),
                ("TRIGGERVERSION", crate::IndexTag::RPMTAG_TRIGGERVERSION as u32),
                ...
                ("CHANGELOGTIME", crate::IndexTag::RPMTAG_CHANGELOGTIME as u32),
                ("CHANGELOGNAME", crate::IndexTag::RPMTAG_CHANGELOGNAME as u32),
                ("CHANGELOGTEXT", crate::IndexTag::RPMTAG_CHANGELOGTEXT as u32),
                ...
                ("VERIFYSCRIPTPROG", crate::IndexTag::RPMTAG_VERIFYSCRIPTPROG as u32),
                ("TRIGGERSCRIPTPROG", crate::IndexTag::RPMTAG_TRIGGERSCRIPTPROG as u32),
                ...
                ("TRIGGERPOSTUN", crate::IndexTag::RPMTAG_TRIGGERPOSTUN as u32),
                ...
                ("SOURCEPACKAGE", crate::IndexTag::RPMTAG_SOURCEPACKAGE as u32),
                ("OLDORIGFILENAMES", crate::IndexTag::RPMTAG_OLDORIGFILENAMES as u32),
                ("BUILDPREREQ", crate::IndexTag::RPMTAG_BUILDPREREQ as u32),
                ("BUILDREQUIRES", crate::IndexTag::RPMTAG_BUILDREQUIRES as u32),
                ("BUILDCONFLICTS", crate::IndexTag::RPMTAG_BUILDCONFLICTS as u32),
                ("BUILDMACROS", crate::IndexTag::RPMTAG_BUILDMACROS as u32),
                ("PROVIDEFLAGS", crate::IndexTag::RPMTAG_PROVIDEFLAGS as u32),
                ("PROVIDEVERSION", crate::IndexTag::RPMTAG_PROVIDEVERSION as u32),
                ("OBSOLETEFLAGS", crate::IndexTag::RPMTAG_OBSOLETEFLAGS as u32),
                ("OBSOLETEVERSION", crate::IndexTag::RPMTAG_OBSOLETEVERSION as u32),
                ...
                ("ORIGDIRINDEXES", crate::IndexTag::RPMTAG_ORIGDIRINDEXES as u32),
                ("ORIGBASENAMES", crate::IndexTag::RPMTAG_ORIGBASENAMES as u32),
                ...
                ("PAYLOADFORMAT", crate::IndexTag::RPMTAG_PAYLOADFORMAT as u32),
                ("PAYLOADCOMPRESSOR", crate::IndexTag::RPMTAG_PAYLOADCOMPRESSOR as u32),
                ...
                ("PATCHESVERSION", crate::IndexTag::RPMTAG_PATCHESVERSION as u32),
                ...
                ("CACHEPKGMTIME", crate::IndexTag::RPMTAG_CACHEPKGMTIME as u32),
                ...
                ("POSTTRANSPROG", crate::IndexTag::RPMTAG_POSTTRANSPROG as u32),
                ("DISTTAG", crate::IndexTag::RPMTAG_DISTTAG as u32),
                ("OLDSUGGESTSNAME", crate::IndexTag::RPMTAG_OLDSUGGESTSNAME as u32),
                ("OLDSUGGESTSVERSION", crate::IndexTag::RPMTAG_OLDSUGGESTSVERSION as u32),
                ("OLDSUGGESTSFLAGS", crate::IndexTag::RPMTAG_OLDSUGGESTSFLAGS as u32),
                ("OLDENHANCESNAME", crate::IndexTag::RPMTAG_OLDENHANCESNAME as u32),
                ("OLDENHANCESVERSION", crate::IndexTag::RPMTAG_OLDENHANCESVERSION as u32),
                ("OLDENHANCESFLAGS", crate::IndexTag::RPMTAG_OLDENHANCESFLAGS as u32),
                ...
                ("PACKAGEORIGIN", crate::IndexTag::RPMTAG_PACKAGEORIGIN as u32),
                ("TRIGGERPREIN", crate::IndexTag::RPMTAG_TRIGGERPREIN as u32),
                ("BUILDSUGGESTS", crate::IndexTag::RPMTAG_BUILDSUGGESTS as u32),
                ("BUILDENHANCES", crate::IndexTag::RPMTAG_BUILDENHANCES as u32),
                ("SCRIPTSTATES", crate::IndexTag::RPMTAG_SCRIPTSTATES as u32),
                ("SCRIPTMETRICS", crate::IndexTag::RPMTAG_SCRIPTMETRICS as u32),
                ("BUILDCPUCLOCK", crate::IndexTag::RPMTAG_BUILDCPUCLOCK as u32),
                ("FILEDIGESTALGOS", crate::IndexTag::RPMTAG_FILEDIGESTALGOS as u32),
                ...
                ("BUILDPLATFORMS", crate::IndexTag::RPMTAG_BUILDPLATFORMS as u32),
                ("PACKAGECOLOR", crate::IndexTag::RPMTAG_PACKAGECOLOR as u32),
                ("PACKAGEPREFCOLOR", crate::IndexTag::RPMTAG_PACKAGEPREFCOLOR as u32),
                ...
                ("CONFLICTATTRSX", crate::IndexTag::RPMTAG_CONFLICTATTRSX as u32),
                ("OBSOLETEATTRSX", crate::IndexTag::RPMTAG_OBSOLETEATTRSX as u32),
                ("PROVIDEATTRSX", crate::IndexTag::RPMTAG_PROVIDEATTRSX as u32),
                ("REQUIREATTRSX", crate::IndexTag::RPMTAG_REQUIREATTRSX as u32),
                ("BUILDPROVIDES", crate::IndexTag::RPMTAG_BUILDPROVIDES as u32),
                ("BUILDOBSOLETES", crate::IndexTag::RPMTAG_BUILDOBSOLETES as u32),
                ...
                ("ORIGFILENAMES", crate::IndexTag::RPMTAG_ORIGFILENAMES as u32),
                ("LONGFILESIZES", crate::IndexTag::RPMTAG_LONGFILESIZES as u32),
                ("LONGSIZE", crate::IndexTag::RPMTAG_LONGSIZE as u32),
                ("FILECAPS", crate::IndexTag::RPMTAG_FILECAPS as u32),
                ("FILEDIGESTALGO", crate::IndexTag::RPMTAG_FILEDIGESTALGO as u32),
                ...
                ("PRETRANSFLAGS", crate::IndexTag::RPMTAG_PRETRANSFLAGS as u32),
                ("POSTTRANSFLAGS", crate::IndexTag::RPMTAG_POSTTRANSFLAGS as u32),
                ("VERIFYSCRIPTFLAGS", crate::IndexTag::RPMTAG_VERIFYSCRIPTFLAGS as u32),
                ("TRIGGERSCRIPTFLAGS", crate::IndexTag::RPMTAG_TRIGGERSCRIPTFLAGS as u32),
                ...
                ("POLICYTYPESINDEXES", crate::IndexTag::RPMTAG_POLICYTYPESINDEXES as u32),
                ...
                ("INSTFILENAMES", crate::IndexTag::RPMTAG_INSTFILENAMES as u32),
                ("REQUIRENEVRS", crate::IndexTag::RPMTAG_REQUIRENEVRS as u32),
                ("PROVIDENEVRS", crate::IndexTag::RPMTAG_PROVIDENEVRS as u32),
                ("OBSOLETENEVRS", crate::IndexTag::RPMTAG_OBSOLETENEVRS as u32),
                ("CONFLICTNEVRS", crate::IndexTag::RPMTAG_CONFLICTNEVRS as u32),
                ("FILENLINKS", crate::IndexTag::RPMTAG_FILENLINKS as u32),
                ("RECOMMENDNAME", crate::IndexTag::RPMTAG_RECOMMENDNAME as u32),
                ("RECOMMENDVERSION", crate::IndexTag::RPMTAG_RECOMMENDVERSION as u32),
                ("RECOMMENDFLAGS", crate::IndexTag::RPMTAG_RECOMMENDFLAGS as u32),
                ("SUGGESTNAME", crate::IndexTag::RPMTAG_SUGGESTNAME as u32),
                ("SUGGESTVERSION", crate::IndexTag::RPMTAG_SUGGESTVERSION as u32),
                ("SUGGESTFLAGS", crate::IndexTag::RPMTAG_SUGGESTFLAGS as u32),
                ("SUPPLEMENTNAME", crate::IndexTag::RPMTAG_SUPPLEMENTNAME as u32),
                ("SUPPLEMENTVERSION", crate::IndexTag::RPMTAG_SUPPLEMENTVERSION as u32),
                ("SUPPLEMENTFLAGS", crate::IndexTag::RPMTAG_SUPPLEMENTFLAGS as u32),
                ("ENHANCENAME", crate::IndexTag::RPMTAG_ENHANCENAME as u32),
                ("ENHANCEVERSION", crate::IndexTag::RPMTAG_ENHANCEVERSION as u32),
                ("ENHANCEFLAGS", crate::IndexTag::RPMTAG_ENHANCEFLAGS as u32),
                ("RECOMMENDNEVRS", crate::IndexTag::RPMTAG_RECOMMENDNEVRS as u32),
                ("SUGGESTNEVRS", crate::IndexTag::RPMTAG_SUGGESTNEVRS as u32),
                ("SUPPLEMENTNEVRS", crate::IndexTag::RPMTAG_SUPPLEMENTNEVRS as u32),
                ("ENHANCENEVRS", crate::IndexTag::RPMTAG_ENHANCENEVRS as u32),
                ("ENCODING", crate::IndexTag::RPMTAG_ENCODING as u32),
                ("FILETRIGGERIN", crate::IndexTag::RPMTAG_FILETRIGGERIN as u32),
                ("FILETRIGGERUN", crate::IndexTag::RPMTAG_FILETRIGGERUN as u32),
                ("FILETRIGGERPOSTUN", crate::IndexTag::RPMTAG_FILETRIGGERPOSTUN as u32),
                ("FILETRIGGERSCRIPTS", crate::IndexTag::RPMTAG_FILETRIGGERSCRIPTS as u32),
                ("FILETRIGGERSCRIPTPROG", crate::IndexTag::RPMTAG_FILETRIGGERSCRIPTPROG as u32),
                ("FILETRIGGERSCRIPTFLAGS", crate::IndexTag::RPMTAG_FILETRIGGERSCRIPTFLAGS as u32),
                ("FILETRIGGERNAME", crate::IndexTag::RPMTAG_FILETRIGGERNAME as u32),
                ("FILETRIGGERINDEX", crate::IndexTag::RPMTAG_FILETRIGGERINDEX as u32),
                ("FILETRIGGERVERSION", crate::IndexTag::RPMTAG_FILETRIGGERVERSION as u32),
                ("FILETRIGGERFLAGS", crate::IndexTag::RPMTAG_FILETRIGGERFLAGS as u32),
                ("TRANSFILETRIGGERIN", crate::IndexTag::RPMTAG_TRANSFILETRIGGERIN as u32),
                ("TRANSFILETRIGGERUN", crate::IndexTag::RPMTAG_TRANSFILETRIGGERUN as u32),
                ("TRANSFILETRIGGERPOSTUN", crate::IndexTag::RPMTAG_TRANSFILETRIGGERPOSTUN as u32),
                ("TRANSFILETRIGGERSCRIPTS", crate::IndexTag::RPMTAG_TRANSFILETRIGGERSCRIPTS as u32),
                ("TRANSFILETRIGGERSCRIPTPROG", crate::IndexTag::RPMTAG_TRANSFILETRIGGERSCRIPTPROG as u32),
                ("TRANSFILETRIGGERSCRIPTFLAGS", crate::IndexTag::RPMTAG_TRANSFILETRIGGERSCRIPTFLAGS as u32),
                ("TRANSFILETRIGGERNAME", crate::IndexTag::RPMTAG_TRANSFILETRIGGERNAME as u32),
                ("TRANSFILETRIGGERINDEX", crate::IndexTag::RPMTAG_TRANSFILETRIGGERINDEX as u32),
                ("TRANSFILETRIGGERVERSION", crate::IndexTag::RPMTAG_TRANSFILETRIGGERVERSION as u32),
                ("TRANSFILETRIGGERFLAGS", crate::IndexTag::RPMTAG_TRANSFILETRIGGERFLAGS as u32),
                ("REMOVEPATHPOSTFIXES", crate::IndexTag::RPMTAG_REMOVEPATHPOSTFIXES as u32),
                ("FILETRIGGERPRIORITIES", crate::IndexTag::RPMTAG_FILETRIGGERPRIORITIES as u32),
                ("TRANSFILETRIGGERPRIORITIES", crate::IndexTag::RPMTAG_TRANSFILETRIGGERPRIORITIES as u32),
                ("FILETRIGGERCONDS", crate::IndexTag::RPMTAG_FILETRIGGERCONDS as u32),
                ("FILETRIGGERTYPE", crate::IndexTag::RPMTAG_FILETRIGGERTYPE as u32),
                ("TRANSFILETRIGGERCONDS", crate::IndexTag::RPMTAG_TRANSFILETRIGGERCONDS as u32),
                ("TRANSFILETRIGGERTYPE", crate::IndexTag::RPMTAG_TRANSFILETRIGGERTYPE as u32),
                ("FILESIGNATURES", crate::IndexTag::RPMTAG_FILESIGNATURES as u32),
                ("FILESIGNATURELENGTH", crate::IndexTag::RPMTAG_FILESIGNATURELENGTH as u32),
                ("PAYLOADSHA256", crate::IndexTag::RPMTAG_PAYLOADSHA256 as u32),
                ("PAYLOADSHA256ALGO", crate::IndexTag::RPMTAG_PAYLOADSHA256ALGO as u32),
                ("AUTOINSTALLED", crate::IndexTag::RPMTAG_AUTOINSTALLED as u32),
                ("IDENTITY", crate::IndexTag::RPMTAG_IDENTITY as u32),
                ("MODULARITYLABEL", crate::IndexTag::RPMTAG_MODULARITYLABEL as u32),
                ("PAYLOADSHA256ALT", crate::IndexTag::RPMTAG_PAYLOADSHA256ALT as u32),
                ("ARCHSUFFIX", crate::IndexTag::RPMTAG_ARCHSUFFIX as u32),
                ("SPEC", crate::IndexTag::RPMTAG_SPEC as u32),
                ("TRANSLATIONURL", crate::IndexTag::RPMTAG_TRANSLATIONURL as u32),
                ("UPSTREAMRELEASES", crate::IndexTag::RPMTAG_UPSTREAMRELEASES as u32),
                ("SOURCELICENSE", crate::IndexTag::RPMTAG_SOURCELICENSE as u32),
                ("PREUNTRANS", crate::IndexTag::RPMTAG_PREUNTRANS as u32),
                ("POSTUNTRANS", crate::IndexTag::RPMTAG_POSTUNTRANS as u32),
                ("PREUNTRANSPROG", crate::IndexTag::RPMTAG_PREUNTRANSPROG as u32),
                ("POSTUNTRANSPROG", crate::IndexTag::RPMTAG_POSTUNTRANSPROG as u32),
                ("PREUNTRANSFLAGS", crate::IndexTag::RPMTAG_PREUNTRANSFLAGS as u32),
                ("POSTUNTRANSFLAGS", crate::IndexTag::RPMTAG_POSTUNTRANSFLAGS as u32),
                ...
                ("PAYLOADSIZEALT", crate::IndexTag::RPMTAG_PAYLOADSIZEALT as u32),
                ("RPMFORMAT", crate::IndexTag::RPMTAG_RPMFORMAT as u32),
                ("FILEMIMEINDEX", crate::IndexTag::RPMTAG_FILEMIMEINDEX as u32),
                ("MIMEDICT", crate::IndexTag::RPMTAG_MIMEDICT as u32),
                ("FILEMIMES", crate::IndexTag::RPMTAG_FILEMIMES as u32),
                ("PACKAGEDIGESTS", crate::IndexTag::RPMTAG_PACKAGEDIGESTS as u32),
                ("PACKAGEDIGESTALGOS", crate::IndexTag::RPMTAG_PACKAGEDIGESTALGOS as u32),
                ("SOURCENEVR", crate::IndexTag::RPMTAG_SOURCENEVR as u32),
                ("PAYLOAD_SHA512", crate::IndexTag::RPMTAG_PAYLOAD_SHA512 as u32),
                ("PAYLOAD_SHA512_ALT", crate::IndexTag::RPMTAG_PAYLOAD_SHA512_ALT as u32),
                ("PAYLOAD_SHA3_256", crate::IndexTag::RPMTAG_PAYLOAD_SHA3_256 as u32),
                ("PAYLOAD_SHA3_256_ALT", crate::IndexTag::RPMTAG_PAYLOAD_SHA3_256_ALT as u32),
            ],
        )?,
    )?;
    ...
    m.add(
        ...
        make_int_enum(
            ...
            "SignatureTag",
            &[
                ("HEADER_SIGNATURES", crate::IndexSignatureTag::HEADER_SIGNATURES as u32),
                ("FILESIGNATURES", crate::IndexSignatureTag::RPMSIGTAG_FILESIGNATURES as u32),
                ("FILESIGNATURE_LENGTH", crate::IndexSignatureTag::RPMSIGTAG_FILESIGNATURE_LENGTH as u32),
                ("VERITYSIGNATURES", crate::IndexSignatureTag::RPMSIGTAG_VERITYSIGNATURES as u32),
                ("VERITYSIGNATUREALGO", crate::IndexSignatureTag::RPMSIGTAG_VERITYSIGNATUREALGO as u32),
                ("OPENPGP", crate::IndexSignatureTag::RPMSIGTAG_OPENPGP as u32),
                ...
                ("SHA3_256", crate::IndexSignatureTag::RPMSIGTAG_SHA3_256 as u32),
                ("SHA256", crate::IndexSignatureTag::RPMSIGTAG_SHA256 as u32),
                ("RESERVED", crate::IndexSignatureTag::RPMSIGTAG_RESERVED as u32),
                ("RESERVEDSPACE", crate::IndexSignatureTag::RPMSIGTAG_RESERVEDSPACE as u32),
                ...
                ("PAYLOADSIZE", crate::IndexSignatureTag::RPMSIGTAG_PAYLOADSIZE as u32),
                ("LONGSIZE", crate::IndexSignatureTag::RPMSIGTAG_LONGSIZE as u32),
                ("LONGARCHIVESIZE", crate::IndexSignatureTag::RPMSIGTAG_LONGARCHIVESIZE as u32),
            ],
        )?,
    )?;
