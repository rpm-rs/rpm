...
#[cfg(all(test, feature = "signing-ring"))]
mod test {
    use super::*;
    ...
    #[cfg(feature = "signing-ring")]
    type Signer = SignerRing;
    #[cfg(feature = "signing-ring")]
    type Verifier = VerifierRing;
    ...
    fn sign_verify_round() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let signer = Signer::load_from_pkcs8 (&gpg_signing_key2[..]).unwrap();
        let signature =  signer.sign(&digest_sha1.bytes()[..]).unwrap();


        let verifyer = Verifier::load_from_pkcs8(&gpg_signing_key[..]).unwrap();
