...
cat >$CONFIG_FILE <<EOF
    ...
    Name-Real: rpm-rs
    Name-Comment: key
    Name-Email: rpm-signing-key-ecdsa-p256@example.com
    Expire-Date: 0
    %no-ask-passphrase
    %no-protection
EOF
...
gpg --output ./signing_keys/public_ecdsa_p256.asc --armor --export rpm-signing-key-ecdsa-p256@example.com
gpg --output ./signing_keys/secret_ecdsa_p256.asc --armor --export-secret-key rpm-signing-key-ecdsa-p256@example.com
